...
async function geminiNextSegment(id, env) {
  ...
  const startS = n * SEG;
  const endS = status.open ? startS + SEG : Math.min((n + 1) * SEG, status.duration_s);
  const glossary = await loadGlossary(env);
...
若此時間段已超出影片實際結尾(影片比預估短),只輸出空陣列 []。`;
  ...
  const CAP = 40; // open 模式上限(2 小時),防失控
  const ended = status.open
    ? (cues.length === 0 || status.done_segments >= CAP)
    : status.done_segments >= status.segments;
  if (status.open && !ended && status.done_segments >= status.segments)
    status.segments = status.done_segments + 1; // 估計太短 → 滾動延伸進度目標
