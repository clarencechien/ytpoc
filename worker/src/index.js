  ...
  async queue(batch, env) {
    for (const msg of batch.messages) {
      ...
      try {
        ...
        if (status.stage === "gemini" && status.fail_count >= 4) {
          // 同一段連炸 4 次(常見:Gemini 500 / 段落超出片尾):跳過此段,任務繼續
          status.skipped = [...(status.skipped || []), status.covered_s ?? 0];
          status.covered_s = (status.covered_s ?? 0) + 180;
          status.done_segments += 1;
          status.fail_count = 0;
          status.last_error = "已跳過 " + status.skipped[status.skipped.length - 1] + "s 起的段落(連續失敗)";
