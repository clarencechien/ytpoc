...
export default {
  async fetch(req, env) {
    ...
    try {
      ...
      if ((m = p.match(/^\/admin\/videos\/([\w-]{11})\/rebuild$/)) && req.method === "POST") {
        const s = await getJSON(env, `videos/${m[1]}/status.json`);
        if (!s || !s.done_segments) return j({ error: "此影片沒有 Gemini 段落可重建" }, 400);
        const cues = await assembleGemini(m[1], env, s.done_segments);
        s.cues = cues;
        await putJSON(env, `videos/${m[1]}/status.json`, s);
        return j({ id: m[1], cues, rebuilt: true });
      }
  ...
  async queue(batch, env) {
    for (const msg of batch.messages) {
      ...
      try {
        ...
        const key = String(status.covered_s ?? status.done_batches ?? 0);
        status.fail_count = status.fail_key === key ? (status.fail_count || 0) + 1 : 1;
        status.fail_key = key;
        ...
        if (status.stage === "gemini" && status.fail_count >= 2 && !status.try_len) {
          ...
          status.try_len = 60;
          status.fail_count = 0;
          status.last_error += "(已降為 60 秒細掃)";
        } else if (status.stage === "gemini" && status.fail_count >= 3 && status.try_len) {
          const tailZone = !status.open && status.duration_s - (status.covered_s ?? 0) <= 360;
          if (status.open || tailZone) {
            // 細掃仍炸且已在片尾附近:判定影片實際到此結束,正確收尾
            status.duration_s = status.covered_s ?? 0;
            status.open = false;
            status.last_error = "片尾偵測:影片實際長度約 " + status.duration_s + "s,收尾合併";
          } else {
            // 中段毒段:只跳 60 秒
            status.skipped = [...(status.skipped || []), status.covered_s ?? 0];
            status.covered_s = (status.covered_s ?? 0) + 60;
            status.done_segments += 1;
            status.last_error = "已跳過 " + status.skipped[status.skipped.length - 1] + "s 起的 60 秒(連續失敗)";
          }
          status.fail_count = 0;
          await putJSON(env, `videos/${id}/status.json`, status);
          await env.JOBS.send({ id });
...
async function geminiNextSegment(id, env) {
  ...
  const SEG = status.try_len || GEMINI_SEG_S; // 失敗後降為 60s 細掃,成功即恢復
...
若此時間段已超出影片實際結尾,只輸出空陣列 []。`;
  ...
  let cues = [];
  if (status.open || startS < status.duration_s - 1) {
  const r = await fetch(
    `https://generativelanguage.googleapis.com/v1beta/models/${env.GEMINI_MODEL}:generateContent?key=${env.GEMINI_API_KEY}`,
    {
      method: "POST",
  ...
  if (ended) {
    ...
    status.cues = await assembleGemini(id, env, status.done_segments);
    status.stage = "done";
  }
  await putJSON(env, `videos/${id}/status.json`, status);
  return j(status);
...
// Gemini 段落合併 + 確定性清洗(壞時間戳/重複內嵌字幕/語言洩漏)
function sanitizeCues(all) {
  all.sort((a, b) => a.start - b.start);
  const norm = s => String(s || "").replace(/^[^|]*\|/, "").replace(/[\s\p{P}\p{S}]+/gu, "");
  ...
  for (const c of all) {
    ...
    if (c.kind === "card") {
      const k = norm(c.ko);
      const dup = k && speech.some(s2 => {
        if (Math.abs(+s2.start - start) > 6) return false;
        const ks = norm(s2.ko);
        return ks && (k.includes(ks) || ks.includes(k) ||
          (ks.length > 10 && k.includes(ks.slice(0, 10))));
      });
      if (dup) continue;
    }
...
async function rebuild(id){
  try { const r = await api('/admin/videos/' + id + '/rebuild', {method:'POST'});
    log('♻ 已重建 ' + id + ':' + r.cues + ' cues', 'ok'); refreshJobs(); }
  catch (e) { log('重建失敗:' + e.message, 'err'); }
}
